...
@section('container')
    ...
    <div class="row justify-content-center align-content-center form-login text-responsive">
        @if (session()->has('failRegister'))
            <div class="row justify-content-center">
                <div class="col-8 col-md-7 col-lg-5 alert alert-success" role="alert">
                    {{ session('failRegister') }}
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                </div>
            </div>
        @endif
        <form action="/register" method="POST" class="col-9 col-md-8 col-lg-6 card py-4 px-4" autocomplete="of">
            @csrf
            <div class="mb-3">
                <label for="name" class="form-label ">Name</label>
                <input name="name" type="text" class="form-control @error('name') is-invalid @enderror" id="name"
                    placeholder="Budi" value="{{ old('name') }}">
                @error('name')
            ...
            <div class="mb-3">
                ...
                <input name="username" type="text" class="form-control @error('username') is-invalid @enderror"
                    id="username" placeholder="budi123" value="{{ old('username') }}">
                @error('username')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
            ...
            <div class="mb-3">
                ...
                <input name="passConfirm" type="password" class="form-control @error('passConfirm') is-invalid @enderror"
                    id="passConfirm" placeholder="">
                @error('passConfirm')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
            ...
            <div class="mb-3">
                ...
                <input name="nik" type="text" class="form-control @error('nik') is-invalid @enderror"
                    id="nik" placeholder="740********" value="{{ old('nik') }}">
                @error('nik')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
            ...
            <div class="mb-3">
                ...
                <input name="telp" type="text" class="form-control @error('telp') is-invalid @enderror"
                    id="telp" placeholder="08********" value="{{ old('telp') }}">
                @error('telp')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
            ...
            <p class="text-center mt-3">Sudah punya akun ? <span class="text-primary"><a href="/login">Login</a></span>
        </form>
    </div>
@endsection
